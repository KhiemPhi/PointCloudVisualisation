...
def main(gpu, cfg):
    ...
    if cfg.distributed:
        ...
        model = nn.parallel.DistributedDataParallel(
            model.cuda(), device_ids=[cfg.rank], output_device=cfg.rank, find_unused_parameters=True)
...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
        ...
        # preds --> {data['cls']: "Airplane", data['pos']:xyz, preds}
        # idx 1: (1,1,1) id: 2
        # idx2: id 4
        # add django_vis variable in default.cfg
        # for coordiantes, label in zip(data['pos'], preds)
        # if cfg.django_vis == True:
        # ---- Input your OWn Code Here -----
        save_points_json(data,preds, idx)
...
import json
def save_points_json(data, preds, idx, file_path="points.json"):
    points_list = []
    ...
    for i in range(len(data['pos'])):
        ...
        predicted_parts_label = preds[i].cpu().numpy().tolist()  # Find the predicted class for this point
        cls_label = data['cls'][i].cpu().numpy().tolist()
        class_name = classes[cls_label[0]]
    
        sample = {
            "coordinates": data['pos'][i].cpu().numpy().tolist(),  # Convert tensor to list
            "part_label": predicted_parts_label,
        ...
        path_name = "{}_{}_points.json".format(class_name, batch_num)
